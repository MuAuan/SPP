...
def save_files(epoch):
    model.save_weights('params_model_epoch_{0:03d}.hdf5'.format(epoch), True)   
    save_history(history, os.path.join(result_dir, 'history_epoch_{0:03d}.txt'.format(epoch)))
...
for i in range(epochs):
    ...
    if not data_augmentation:
        ...
        csv_logger = keras.callbacks.CSVLogger('history.log', separator=',', append=True)  #CSVLogger('history.log')
        ...
        csv_logger = csv_logger = keras.callbacks.CSVLogger('history.log', separator=',', append=True) #CSVLogger('history.log')
        callbacks = [early_stopping, lr_reduction, csv_logger]
